...
| `supabase/migrations/005_speaking.sql` | **신규** usage·attempts 테이블 + 시드 |
| `scripts/generate_speaking_seed.py` | **신규** 시나리오 배치 → SQL |

## 3. Supabase (migration 005)
...
- [ ] migration 005 + 시나리오 10개 시드 (human review)
- [ ] STT 어댑터 (on-device 검증, Whisper 폴백 경로)
- [ ] Edge Function 배포 + secret 설정 + deno test
- [ ] 일일 10회 cap 동작 (429 → 업그레이드 안내 UI)
- [ ] 대화 화면: TTS 대사 → STT → 피드백 → 다음 턴
- [ ] Dev Mock 모드 전체 플로우
- [ ] speaking_attempts/sessions 기록 + XP
- [ ] 실기기 검증 (권한·지연·비용 로그)
